                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {isHackmon && (
                                <div className="flex gap-1.5 ml-2">
                                    {[0, 1].map(i => <select key={i} value={customT[i] || ''} onChange={e => { const nT = [...customT]; nT[i] = e.target.value; updatePk({...pk, customTypes: nT.filter(Boolean)}); }} className="bg-purple-50 border-2 border-purple-200 rounded-lg text-[10px] text-purple-600 uppercase font-black px-2 py-1 outline-none shadow-sm"><option value="">{i === 1 ? '---' : 'Tipo'}</option>{TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select>)}
                                </div>
                            )}
                        </div>
