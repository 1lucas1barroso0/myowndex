...
export default function LocalDicePanel({ context="guia", onRoll, compact=false }) {
    ...
    return <section className={`local-dice-panel ${compact ? "is-compact" : "game-panel"}`} aria-label="Rolagens locais">
        <header className="local-dice-heading"><div><span className="screen-eyebrow">Neste aparelho</span><h3>Seus dados. Sua próxima ação.</h3></div><span className="local-dice-source">Web Crypto local</span></header>
        <p className="local-dice-intro">Role no próprio aparelho, mesmo sem internet. Cada resultado fica registrado.</p>
        <form onSubmit={roll} onKeyDown={event=>{if(event.key==="Enter" && event.repeat) event.preventDefault();}}>
            <fieldset disabled={busy}><legend className="sr-only">Configurar rolagem local</legend>
                <div className="local-dice-tabs" aria-label="Tipo de rolagem">{[["attribute","Teste · 2d6"],["percent","Chance · d100"],["free","Dados livres"]].map(([kind,label])=><button type="button" key={kind} aria-pressed={draft.kind===kind} onClick={()=>update("kind",kind)}>{label}</button>)}</div>
                {draft.kind!=="free" && <div className="local-dice-modes" aria-label="Modo da rolagem">{Object.entries(LOCAL_ROLL_MODES).map(([mode,label])=><button type="button" key={mode} aria-pressed={draft.mode===mode} onClick={()=>update("mode",mode)}>{label}</button>)}</div>}
        ...
        {result && !result.legacy && <article className="local-dice-result" key={result.id} aria-live="polite" aria-atomic="true">
            ...
            <div className="local-dice-verdict">{result.critical && <b>Crítico</b>}{result.fumble && <b>Erro crítico</b>}{result.success!==null && <strong>{result.success ? "Sucesso" : "Falha"}{result.spec.kind==="percent" ? ` · chance base ${result.spec.chance}%` : ` · dificuldade ${result.spec.opposition}${result.margin===0 ? " (empate)" : ""}`}</strong>}</div>
            ...
            <details className="local-dice-receipt"><summary>Conferir esta rolagem</summary><p>{new Date(result.createdAt).toLocaleString("pt-BR")} · {result.context}</p><code>{result.id}</code><p>Web Crypto com rejeição de viés. O MyOwnDex nunca troca resultados para interromper uma sequência. A apresentação não sorteia valores extras.</p></details>
        </article>}
        <details className="local-dice-history"><summary>Conferir sequência deste aparelho <b>{history.length}/100</b></summary><div>
