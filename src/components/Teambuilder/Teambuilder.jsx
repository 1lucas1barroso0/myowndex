...
export default function Teambuilder({ envProps }) {
    ...
    // Controles da nova Cápsula de Dados do Cabo Link
    ...
    const generateLinkCode = () => {
        try {
            // Nova compressão otimizada: Apenas o DNA do parceiro!
            const slimTeam = {
                n: active.name,
                p: active.pokemon.map(pk => ({
                    n: pk.species?.name,
                    l: pk.level, i: pk.item, a: pk.ability, na: pk.nature, m: pk.moves,
                    iv: pk.ivs, ev: pk.evs, gx: pk.canGMax, tt: pk.teraType, f: pk.friendship,
                    cs: pk.customStats, ct: pk.customTypes
                }))
            };
            const code = btoa(encodeURIComponent(JSON.stringify(slimTeam)));
            setShareCode(`MYOWNDEX-${code}`);
            setCopied(false);
        } catch { /* Ignora falha de compactação local */ }
    };
    ...
    const receiveViaLinkCable = async () => {
        try {
            ...
            // Reconstrução Genética na Pokédex local
            const reconstructed = await Promise.all(pkmns.map(async (pk) => {
                if (pk.species && pk.species.sprites) return pk; // Formato antigo ainda é suportado
    ...
    if (!teams.length) return (
        <div className="flex flex-col items-center justify-center py-20 animate-fade-in text-center max-w-lg mx-auto">
            ...
            <p className="text-sm text-slate-500 mb-8 leading-relaxed font-semibold">Inicie uma nova formação ou conecte o seu Cabo Link para receber parceiros de outro Treinador.</p>
            ...
            <div className="flex flex-col gap-4 w-full">
                ...
                {importing ? (
                    <div className="p-5 bg-blue-50 border-2 border-blue-200 rounded-2xl shadow-inner animate-fade-in text-left">
                        ...
                        <input type="text" disabled={isProcessing} value={importData} onChange={e => { setImportData(e.target.value); setImportError(false); }} className="w-full p-3 rounded-xl border-2 border-blue-200 text-xs font-bold text-slate-700 outline-none mb-2 focus:border-blue-500 shadow-inner" />
                        ...
                        {isProcessing ? (
                            <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest block text-center py-2 animate-pulse">Reconstruindo Dados Celulares...</span>
                        ) : (
                ...
                <div className="mt-4 pt-4 border-t-2 border-slate-100">
                    {importing ? (
                        <div className="p-4 bg-blue-50 border-2 border-blue-200 rounded-2xl shadow-inner animate-fade-in">
                            <span className="text-[9px] font-black text-blue-500 uppercase tracking-widest mb-2 block">Cabo Link</span>
                            ...
                            {isProcessing ? (
                                <span className="text-[9px] font-black text-blue-500 uppercase tracking-widest block text-center py-2 animate-pulse">Transferindo...</span>
                            ) : (
                                <div className="flex gap-2">
                    ...
                    <div className="bg-white border-4 border-slate-200 p-6 md:p-8 rounded-3xl shadow-[0_8px_0_#cbd5e1]">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 border-b-4 border-slate-100 pb-5 gap-4">
                            ...
                            <div className="flex gap-3 w-full sm:w-auto">
                                <button onClick={generateLinkCode} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-3.5 bg-white text-blue-500 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 rounded-2xl transition-colors border-2 border-slate-200 shadow-sm outline-none" title="Gerar Código do Cabo Link">
                        ...
                        {shareCode && (
                            <div className="mb-8 p-5 bg-blue-50 border-2 border-blue-200 rounded-2xl flex flex-col sm:flex-row gap-4 items-center justify-between shadow-inner animate-fade-in">
                                <div className="flex-1 w-full">
                                    <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest mb-2 block">Cápsula de Transferência Gerada</span>
                            ...
                            {active.pokemon?.map((pk, i) => (
                                <div key={i} onClick={() => setEditingSlot(i)} className={`p-4 rounded-2xl border-2 cursor-pointer flex gap-4 items-center transition-all relative overflow-hidden group shadow-sm ${editingSlot === i ? 'bg-blue-50 border-blue-400 shadow-[0_4px_0_#60a5fa] translate-y-[-2px]' : 'bg-slate-50 border-slate-200 hover:border-blue-300 hover:bg-white'}`}>
                                    ...
                                    <div className="w-16 h-16 bg-white rounded-xl border-2 border-slate-100 flex items-center justify-center shadow-inner relative z-10 flex-shrink-0">
                                        {pk.species?.sprites?.front_default ? <img src={pk.species.sprites.front_default} className="w-14 h-14 pixelated drop-shadow-md group-hover:scale-110 transition-transform" /> : <span className="text-[9px] font-black text-slate-400">Ausente</span>}
                                    </div>
